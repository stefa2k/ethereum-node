    ...
    async importKey(files, password) {
        ...
        switch (service) {
            case 'prysm':
                ...
                if ((await this.nodeConnection.sshService.exec(StringUtils.suWrap(`cat ${passwords_path}/wallet-password`))).rc === 1) {
                    log.error("No Wallet found")
                    log.info("Generating one")
                    //generate wallet password
                    await this.nodeConnection.sshService.exec(StringUtils.suWrap(`echo ${StringUtils.createRandomString()} > ${passwords_path}/wallet-password`))
                    await this.nodeConnection.sshService.exec(StringUtils.suWrap(`chmod 700 ${passwords_path}/wallet-password`))
                    await this.nodeConnection.sshService.exec(StringUtils.suWrap(`chown 2000:2000 ${passwords_path}/wallet-password`))
                    //Prysm - Create wallet for account(s)
                    await this.nodeConnection.sshService.exec(StringUtils.suWrap(`docker exec stereum-${client.id} /app/cmd/validator/validator wallet create --wallet-dir=/opt/app/data/wallets --wallet-password-file=/opt/app/data/passwords/wallet-password --accept-terms-of-use --keymanager-kind=direct --prater`))

                    await this.nodeConnection.sshService.exec(StringUtils.suWrap(`chown -R 2000:2000 ${wallet_path}`))
                    ...
                    await this.nodeConnection.sshService.exec(StringUtils.suWrap(`chmod 600 ${wallet_path}/direct/accounts/all-accounts.keystore.json`))
                    await Sleep(180000)
                }
                break;
            ...
            case 'nimbus':
                ...
                if ((await this.nodeConnection.sshService.exec(StringUtils.suWrap(`cat ${validator_path}/api-token.txt`))).rc === 1) {
                    log.error("Couldn't read API-Token")
                    log.info("Generating one")
                    await this.nodeConnection.sshService.exec(StringUtils.suWrap(`echo ${StringUtils.createRandomString()} > ${validator_path}/api-token.txt`))
                    await Sleep(180000)
                }
                break;
            ...
            case 'teku':
                ...
                if((await this.nodeConnection.sshService.exec(StringUtils.suWrap(`cat ${dataDir}/teku_api_password.txt`))).rc === 1){
                    log.error("Couldn't read API-Token")
                    log.info("Generating one")
                    const password = StringUtils.createRandomString()
                    await this.nodeConnection.sshService.exec(StringUtils.suWrap('apt install -y openjdk-8-jre-headless'))
                    await this.nodeConnection.sshService.exec(StringUtils.suWrap(`echo ${password} > ${dataDir}/teku_api_password.txt`))
                    await this.nodeConnection.sshService.exec(StringUtils.suWrap(`cd ${dataDir} && keytool -genkeypair -keystore teku_api_keystore -storetype PKCS12 -storepass ${password} -keyalg RSA -keysize 2048 -validity 109500 -dname 'CN=localhost, OU=MyCompanyUnit, O=MyCompany, L=MyCity, ST=MyState, C=AU' -ext san=dns:localhost,ip:127.0.0.1`))
                    await Sleep(30000)
                }
                break;
